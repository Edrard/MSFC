...
function check_sql_303()
{
    global $db;

    $sql = "SELECT value FROM `config` WHERE name = 'company';";
    $q = $db->prepare($sql);
    if ($q->execute() == TRUE) {
        $check_if = $q->fetch(PDO::FETCH_NUM);
    } else {
        die(show_message($q->errorInfo(),__line__,__file__,$sql));
    }
    if($check_if == 0){
        $sql = "INSERT INTO `config` (name,value) VALUES ('company','0');";;
        $q = $db->prepare($sql);
        if ($q->execute() != TRUE) {
            die(show_message($q->errorInfo(),__line__,__file__,$sql));
        }  
        $sql = "INSERT INTO `config` (name,value) VALUES ('company_count','1');";;
        $q = $db->prepare($sql);
        if ($q->execute() != TRUE) {
            die(show_message($q->errorInfo(),__line__,__file__,$sql));
        } 
